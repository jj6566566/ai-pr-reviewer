...
export async function fetchRepos(q?: string): Promise<Repo[]> {
  ...
  if (q) params.set("q", q)
  const res = await fetch(`${API_BASE}/github/repos?${params}`, { headers: authHeaders() })
  if (!res.ok) throw new Error("获取仓库列表失败")
  return res.json()
}

export async function fetchRepoPulls(owner: string, repo: string): Promise<PullRequest[]> {
  const res = await fetch(`${API_BASE}/github/repos/${owner}/${repo}/pulls`, { headers: authHeaders() })
