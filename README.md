...
**一款注重评审质量与用户体验的 AI 代码评审工具。** 输入 PR 链接，系统自动拉取代码变更，调用大语言模型进行深度分析。

### 核心特色

🚀 **意图一致性检查** — 首创功能！AI 自动对比 PR 描述与实际代码变更，识别范围漂移、隐含破坏性改动等「名不副实」的问题。

🎯 **风险置信度评估** — 每条风险项都经过置信度计算，自动标记疑似误报，配合用户反馈形成闭环优化。

⚡ **智能缓存 + 秒级响应** — 24h 内同一 PR 免重复分析，追加写入支持历史对比，省 Token、秒出结果。

📝 **GitHub 评论发布** — 评审完成后一键将风险摘要发布为 PR 评论，团队成员无需登录即可在 GitHub 查看评审结果。

支持单 PR / 批量分析、历史趋势洞察、评审报告导出（Markdown/Word/PDF）等完整工作流。
...
### 评审队列
历史评审记录搜索筛选，按风险等级过滤。支持对风险项和建议提交反馈：
- **即时反馈高亮**：点击采纳/误报按钮立即高亮显示，带环形边框
- **状态实时显示**：右侧显示「处理中」「已采纳」「已标记误报」「反馈已取消」等状态
- **一键取消反馈**：再次点击已确认的反馈可取消，支持二次确认
