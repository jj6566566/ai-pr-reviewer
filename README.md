...
- **GitHub 评论发布** — 评审完成后可一键将风险摘要发布为 PR 评论，团队成员无需登录系统即可在 GitHub 上查看评审结果
...
- **风险分布条**（各 PR 的风险占比可视化）
- **跨 PR 文件重叠检测**：识别多个 PR 同时修改同一文件的情况
- **跨 PR 重复风险检测**：自动识别多个 PR 中重复出现的风险项和相似代码片段
- **相似代码块检测**：发现不同 PR 中的代码重复模式
- **每个 PR 的折叠详情面板**：独立查看每个 PR 的完整分析结果
- **并发分析优化**：多个 PR 同时异步分析，充分利用 API 并发能力
...
在调用 AI 模型评审前，系统通过以下步骤构建高质量的上下文：

1. **GitHub API 拉取** — 获取 PR 元信息（标题、描述、分支）、文件变更列表和完整 `diff`
2. **变更类型识别** — 自动识别 `db_schema`（数据库 Schema 变更）、`config`（配置变更）、`auth`（认证变更）、`api_breaking`（API 破坏性变更）等高敏感变更类型
3. **优先级文件排序** — 将业务逻辑、API 路由、核心配置等关键文件排在 diff 最前面，文档、测试、构建脚本等放在后面
4. **低优文件截断** — 对大型 diff 进行智能截断，保留高优文件完整内容，避免超过模型上下文窗口
5. **结构化 Prompt** — 将 PR 信息 + diff 内容 + 自定义规则组合为结构化 System Prompt + User Prompt
6. **风险聚类归并** — 分析完成后按严重度、文件路径和语义相似度对风险项进行聚类，减少重复输出
7. **置信度与误报检测** — 自动计算每条风险的置信度评分，标记疑似误报项供人工核对

### 意图一致性检查

在 PR 分析中，我们增加了一个独特的「意图一致性检查」环节：AI 会将 PR 标题和描述中声明的意图，与实际代码变更的范围进行对比，给出 0-100 分的一致性评分。判定结果分为 "match"（一致）、"minor_deviation"（轻微偏差）、"major_deviation"（重大偏差），并自动识别范围漂移、隐含破坏性改动、描述遗漏、无关文件等问题类型。

> 典型场景：开发者写了"修复登录页面样式"的 PR 描述，但代码里却改了支付模块——这种不一致会被及时发现并提醒评审者关注。
